#pragma once
// SHA-256 — public domain, single-header implementation
...
inline void sha256_final(SHA256_CTX* ctx, uint8_t* hash){
    ...
    ctx->data[63]= ctx->bitlen      &0xff;ctx->data[62]=(ctx->bitlen>> 8)&0xff;
    ctx->data[61]=(ctx->bitlen>>16)&0xff;ctx->data[60]=(ctx->bitlen>>24)&0xff;
    ctx->data[59]=(ctx->bitlen>>32)&0xff;ctx->data[58]=(ctx->bitlen>>40)&0xff;
    ctx->data[57]=(ctx->bitlen>>48)&0xff;ctx->data[56]=(ctx->bitlen>>56)&0xff;
    sha256_transform(ctx,ctx->data);
    for(i=0;i<4;++i){
