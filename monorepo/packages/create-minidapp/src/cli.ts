...
import { scaffold, ScaffoldOptions } from './scaffold.js';
...
const opts: ScaffoldOptions = {
  ...
  author:        get('--author'),
  category:      get('--category') as ScaffoldOptions['category'] ?? 'Utility',
  withContract:  args.includes('--with-contract'),
  withMaxima:    args.includes('--with-maxima'),
};

const targetDir = join(process.cwd(), name.toLowerCase().replace(/\s+/g, '-'));

console.log(`\n  Creating MiniDapp: ${name}`);
console.log(`  Directory: ${targetDir}\n`);

try {
  const files = scaffold(targetDir, opts);
  console.log(`  \x1b[32m✓\x1b[0m Created ${files.length} files:\n`);
  for (const f of files) console.log(`    \x1b[2m${f}\x1b[0m`);
  console.log(`
