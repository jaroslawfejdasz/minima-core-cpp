...
import { MiniValue, TransactionContext, ContractExecution, ContractResult } from './types.js';

type StackItem = MiniValue;
...
function sha3_256hex(data: string): string {
  let h = 0xDEADBEEF;
  for (let i = 0; i < data.length; i++) {
...
export class KissVMInterpreter {
  ...
  private vars: Map<string, StackItem> = new Map();
  private trace: string[] = [];
  private ctx: TransactionContext;
  private instructions = 0;
  private readonly MAX_INSTRUCTIONS = 1024;
  private readonly MAX_STACK = 64;
  ...
  execute(script: string): ContractExecution {
    ...
    try {
      ...
      const contractResult: ContractResult = top === true || top === 'TRUE' ? 'TRUE' : 'FALSE';
  ...
  private runTokens(tokens: string[], startIdx: number): number {
    let i = startIdx;
    while (i < tokens.length) {
      ...
      // ── Control flow ────────────────────────────────────────────
      if (tok === 'IF') {
        const cond = this.pop() as boolean;
        if (cond) {
      ...
      if (tok === 'LET') {
        const name = tokens[i++];
        const eq = tokens[i++]; // '='
        if (eq !== '=') throw new Error(`LET: expected '=', got '${eq}'`);
        const expr = tokens[i++];
  ...
  private runBranch(tokens: string[], startIdx: number, runThen: boolean): number {
    let depth = 1;
    let i = startIdx;
    let elseIdx = -1;
    let endifIdx = -1;
    ...
    if (endifIdx === -1) throw new Error('IF without ENDIF');
    ...
    if (runThen) {
      ...
      if (elseKeyword === 'ELSEIF') {
        ...
        this.runBranch(tokens, innerStart, cond as boolean);
      } else {
  ...
  private evalExpr(expr: string, tokens: string[], _idx: number): MiniValue {
    const up = expr.toUpperCase();
    // Boolean literals
    if (up === 'TRUE') return true;
    if (up === 'FALSE') return false;
    // Numeric
    if (/^-?\d+(\.\d+)?$/.test(expr)) return BigInt(Math.trunc(parseFloat(expr)));
    // Hex
    if (/^0x[0-9a-fA-F]+$/.test(expr)) return expr.toLowerCase();
    // String literal
  ...
  private pushLiteralOrExec(tok: string, _raw: string, tokens: string[], _nextIdx: number): { consumed: boolean; extra: number } {
    const up = tok.toUpperCase();
    // Boolean
    if (up === 'TRUE') { this.push(true); return { consumed: true, extra: 0 }; }
    if (up === 'FALSE') { this.push(false); return { consumed: true, extra: 0 }; }
    // Number
    if (/^-?\d+(\.\d+)?$/.test(tok)) { this.push(BigInt(Math.trunc(parseFloat(tok)))); return { consumed: true, extra: 0 }; }
    // Hex
    if (/^0x[0-9a-fA-F]+$/i.test(tok)) { this.push(tok.toLowerCase()); return { consumed: true, extra: 0 }; }
    // String literal
    ...
    // Variable
    if (this.vars.has(up)) { this.push(this.vars.get(up)!); return { consumed: true, extra: 0 }; }

    // Operators (two-operand, postfix — pop two items)
    if (up === 'ADD') { const b = this.popNum(), a = this.popNum(); this.push(a + b); return { consumed: true, extra: 0 }; }
    if (up === 'SUB') { const b = this.popNum(), a = this.popNum(); this.push(a - b); return { consumed: true, extra: 0 }; }
    if (up === 'MUL') { const b = this.popNum(), a = this.popNum(); this.push(a * b); return { consumed: true, extra: 0 }; }
    if (up === 'DIV') { const b = this.popNum(), a = this.popNum(); if (b === 0n) throw new Error('DIV by zero'); this.push(a / b); return { consumed: true, extra: 0 }; }
    if (up === 'MOD') { const b = this.popNum(), a = this.popNum(); this.push(a % b); return { consumed: true, extra: 0 }; }
    if (up === 'INC') { this.push(this.popNum() + 1n); return { consumed: true, extra: 0 }; }
    if (up === 'DEC') { this.push(this.popNum() - 1n); return { consumed: true, extra: 0 }; }
    if (up === 'NEG') { this.push(-this.popNum()); return { consumed: true, extra: 0 }; }

    // Boolean operators
    if (up === 'AND') { const b = this.popBool(), a = this.popBool(); this.push(a && b); return { consumed: true, extra: 0 }; }
    if (up === 'OR')  { const b = this.popBool(), a = this.popBool(); this.push(a || b); return { consumed: true, extra: 0 }; }
    if (up === 'NOT') { this.push(!this.popBool()); return { consumed: true, extra: 0 }; }
    if (up === 'XOR') { const b = this.popBool(), a = this.popBool(); this.push(a !== b); return { consumed: true, extra: 0 }; }
    if (up === 'NAND') { const b = this.popBool(), a = this.popBool(); this.push(!(a && b)); return { consumed: true, extra: 0 }; }
    if (up === 'NOR')  { const b = this.popBool(), a = this.popBool(); this.push(!(a || b)); return { consumed: true, extra: 0 }; }

    // Comparison
    if (up === 'EQ')  { const b = this.pop(), a = this.pop(); this.push(a === b); return { consumed: true, extra: 0 }; }
    if (up === 'NEQ') { const b = this.pop(), a = this.pop(); this.push(a !== b); return { consumed: true, extra: 0 }; }
    if (up === 'LT')  { const b = this.popNum(), a = this.popNum(); this.push(a < b); return { consumed: true, extra: 0 }; }
    if (up === 'GT')  { const b = this.popNum(), a = this.popNum(); this.push(a > b); return { consumed: true, extra: 0 }; }
    if (up === 'LTE') { const b = this.popNum(), a = this.popNum(); this.push(a <= b); return { consumed: true, extra: 0 }; }
    if (up === 'GTE') { const b = this.popNum(), a = this.popNum(); this.push(a >= b); return { consumed: true, extra: 0 }; }
  ...
  private callBuiltin(name: string, _tokens: string[], _idx: number): MiniValue {
    const ctx = this.ctx;
    switch (name) {
      // ── Transaction context ────────────────────────────────────
      case '@AMOUNT':
      ...
      case 'MULTISIG': {
        ...
        for (let k = 0; k < total; k++) addresses.push(String(this.pop()));
        let count = 0;
        for (const addr of addresses) {
          if (ctx.signers.some(s => s.address === addr || s.publicKey === addr)) count++;
        }
      ...
      case 'LEN': {
        const data = this.pop();
        if (typeof data === 'string') return BigInt(data.replace(/^0x/, '').length / 2);
        return 0n;
      }
      case 'REV': {
  ...
  private pop(): StackItem {
    if (this.stack.length === 0) throw new Error('KISS VM: stack underflow');
