 ...
 */
export function HTLC(
  receiverPublicKey: string,
  senderPublicKey: string,
  hashlock: string,
  refundBlock: number,
  hashFn: 'SHA2' | 'SHA3' = 'SHA2'
): string {
...
function validatePubKey(val: string, name: string) {
...
function validateHash(val: string) {
  if (!val.startsWith('0x')) throw new Error('hashlock must start with 0x');
  if (val.length - 2 !== 64) throw new Error('hashlock must be 32 bytes (64 hex chars) — use SHA2 or SHA3 output');
}
