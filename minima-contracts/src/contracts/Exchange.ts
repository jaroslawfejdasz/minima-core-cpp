...
export function Exchange(
  ...
  if (wantAmount <= 0) throw new Error('Exchange: wantAmount must be > 0');
...
export function LimitOrder(
  ...
  validateHex(wantTokenId, 'wantTokenId');
  if (rate <= 0) throw new Error('LimitOrder: rate must be > 0');
  validateHex(sellerAddress, 'sellerAddress');
  validatePubKey(ownerPubKey);
...
function validateHex(val: string, name: string) {
  if (!val.startsWith('0x')) throw new Error(`${name} must start with 0x`);
  if (!/^0x[0-9a-fA-F]+$/.test(val)) throw new Error(`${name} contains invalid hex`);
}

function validatePubKey(val: string) {
  if (!val.startsWith('0x')) throw new Error('ownerPubKey must start with 0x');
  if (val.length - 2 !== 64) throw new Error('ownerPubKey must be 32 bytes (64 hex chars)');
}
